...
def build_menu_buttons(self, ctx):
    ...
    self.button_selected_uniform = self.button_shader['selected']
...
def renderer_init(self, screenX, screenY):
    ...
    self.directional_light_uniform = self.prog['directional_light']
    self.set_light_direction()
    self.point_light_pos = self.prog['point_light_pos']
    self.point_light_pos.value = (0,0,0)
    self.prog_depth_mvp = self.prog['Depth_MVP']
    self.prog['Texture'].value = 0
    self.prog['shadow_map'].value = 1
    self.prog['bias_matrix'].value = (0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.5, 0.5, 0.5, 1.0)
    ...
    self.shadow_mvp = self.shadow_shader['depthMVP']
    ...
    self.frame_z_val_uniform = self.frame_shader['z_val']
...
class Renderer:
    ...
    def render_ingame(self, data_package):
        ...
        if type(world_ )!= type(None):
            ...
            shadow_mvp = tuple(np.array(self.build_shadow_mvp(local_player)).flatten('F'))#still need to update the mvp each frame
    ...
    def set_MVP_from_model(self, model_matrix):
        ...
        self.mvp_uniform.value = tuple(np.array(mvp).flatten('F'))
